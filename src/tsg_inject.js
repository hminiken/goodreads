// This script is run when visiting the story graph page
...
function sortRowsByStatus() {
    ...
    document.querySelectorAll("bookalike").forEach((element) => {
...
function getOverdriveAvailability() {
    ...
    //Mine
    // StoryGraph removed the old ".to-read-books" wrapper class, so detect the
    // to-read page by URL instead and confirm book panes are actually present.
    var tbr = /\/to-read\//.test(window.location.pathname)
        ? document.querySelectorAll('[class*="book-pane break-words"]')
        : [];
    console.log("tbr is: ");
    console.log(tbr);
    var single_book_view = document.querySelectorAll('.storygraph-preview-pane')
    var browse_books = document.querySelectorAll('.search-results-books')
    console.log("single_book_view: ");
    console.log(browse_books)
    ...
    if (showOnPages["descriptionPage"] && book && !document.querySelector("div#AGtable")) {
    ...
    } else if ((tbr && tbr.length > 0) || (browse_books && browse_books.length > 0)) { //mine: if on tsg tbr page
        // const id = "TBR";
        console.log("TBR")
        console.log(document.querySelectorAll("book-pane:not(.AGseen)"));
		// iterate through every listing in the list that we haven't seen before
        document.querySelectorAll(' [class*="book-pane break-words"]:not(.AGseen)').forEach((element) => {
			...
			// set a "Loading..." message for this listing
			// var avg_col = element.querySelector("td.avg_rating");
			element.insertAdjacentHTML("afterend", createSingleBookPageTable(headerText, id));
			// mark the row as seen
			element.classList.add("AGseen");
            ...
            book_details = element.querySelectorAll(' [class*="book-title-author-and-series"]')[0]
            console.log("Book details:")
            // Prefer the title link inside the <h3> - when a book is part of a series,
            // the first <a> in this block is actually the series link, not the book title.
            const titleLink = book_details.querySelector('h3 a[href*="/books/"]') || book_details.querySelector('a');
            book_title = titleLink.innerText;
            book_author = book_details.querySelector('[class*="font-body"]').innerText;
            ...
            chrome.runtime.sendMessage({
                type: "FROM_AG_PAGE",
                id: id,
                title: cleanTitleForSearch(book_title),
                author: cleanAuthorForSearch(book_author)
            });

			libraryClassNames.forEach(function(className) {
                console.log("libraryClassNames")
				element.classList.add(className);
			})
			waitingOnAvailability = true;
		});
		...
		if (tableUpdateCheckInterval == null) {
			tableUpdateCheckInterval = setInterval(function() {
                console.log("tableUpdateCheckInterval")
				if ( document.querySelectorAll(' [class*="book-pane break-words"]:not(.AGseen)').length > 0) {
					getOverdriveAvailability();
				}
				// sort rows by availability if necessary
				if (waitingOnAvailability) {
    ...
    } else if (single_book_view && single_book_view.length > 0) { //mine: if on tsg single book page
        // var id = "singleBookView";
        console.log("singleBookView")
		// iterate through every listing in the list that we haven't seen before
         document.querySelectorAll('[class*="book-cover"]:not(.AGseen)').forEach((element) => {
             var info_elem = document.querySelector('[class*="book-title-author-and-series"]');
             var book_title_full = info_elem.querySelector('h3').innerHTML;
             
             var book_title = book_title_full.split("\n")[0];
             console.log("BOOK TITLE: " + book_title)
             const id = book_title;
            var book_author = info_elem.querySelector('[class*="font-body"]').innerText;
			// const id = element.getAttribute("data-book-id");
            // console.log(id);
			var oneLine = "";
			if (showFormat.oneLine) {
        ...
        if (tableUpdateCheckInterval == null) {
            tableUpdateCheckInterval = setInterval(function () {
                if (document.querySelectorAll('[class*="book-cover"]:not(.AGseen)').length > 0) {
                    getOverdriveAvailability();
                }
                // sort rows by availability if necessary
                if (waitingOnAvailability) {
...
function injectAvailableReads() {
    ...
    if (!loaded) {
        ...
        document.getElementsByTagName('body')[0].insertAdjacentHTML("beforebegin", `<style>
					...
					.ARimg{text-wrap:nowrap;padding:0px;display:block;}
					...
					.ARshowMoreInSinglePage { position:absolute; right: 15px; bottom: 8px; height:auto; padding-right: 0px} 
...
function insertRow(id, imgCol, descCol, sortScore, hideNotFoundIfOtherResults, notFoundOrder) {
    ...
    console.log("Made it to insert row here");
    console.log(rowDiv);
